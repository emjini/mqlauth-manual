...
5. **認証頻度を勝手に上げない。** MQLAuthサーバーは、短時間に過剰な認証アクセスを行うクライアントの利用を制限する場合がある（判定基準は非公開）。標準実装は初回認証後ローカルキャッシュを使う設計になっており、これを変更しない
...
- **「認証アクセス過多です」のアラート** → サーバー側のアクセス制限に達している（判定基準は非公開）。認証頻度を上げる改造をしていないか、標準のローカルキャッシュを無効化していないかを確認し、時間を置いてから再試行
