...
5. **体験版機能（オプション）** — 未登録口座を初回起動時に自動登録し、指定日数だけ利用可能にする（利用期限は最長で現在から1ヶ月後まで。超える指定はサーバー側で調整される）
...
3. （任意）体験版機能を使うか、使う場合の日数（最長で現在から1ヶ月後まで）。指定がなければ体験版オフで実装する
...
| `_day` | `0` | 体験版の日数（利用期限は最長で現在から1ヶ月後まで。超える指定はサーバー側で調整される） |
